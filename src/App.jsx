...
// football-data.org match → normalised shape
function normaliseMatch(m) {
  ...
  const stage   = stageFromStage(m.stage || m.group || "");
  const elapsed = m.minute || null;
  return { id: String(m.id), home, away, homeGoals, awayGoals, winner, stage, elapsed, date: m.utcDate, venue: m.venue || "", status: m.status };
}
...
function Countdown() {
  ...
  return (
    <div style={{ textAlign:"center", padding:"40px 16px 32px" }}>
      ...
      <div style={{ fontFamily:"'Inter',sans-serif", fontSize:12, color:"#444", marginTop:5 }}>Live scores will auto-populate once the tournament begins ⚽</div>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:16, overflowX:"auto", paddingBottom:2 }}>
          {tabs.map(([t,label]) => (
            <button key={t} className="tab-btn" onClick={() => setTab(t)}
              style={{ flex:"0 0 auto", padding:"9px 10px", borderRadius:8, letterSpacing:1, fontSize:11, whiteSpace:"nowrap",
            ...
            {liveMatches.length === 0 ? (
              ...
              <div key={m.id} className="match-card" style={{ background:"rgba(255,77,77,0.05)", border:"1px solid rgba(255,77,77,0.18)" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                  <TeamBlock name={m.home} align="left" />
                  <div style={{ textAlign:"center", minWidth:72 }}>
                    <div style={{ fontSize:26, letterSpacing:3, color:"#fff" }}>
                  ...
                  <TeamBlock name={m.away} align="right" />
                </div>
              </div>
            ))}
          </div>
        ...
        {tab==="schedule" && (
          <div className="fade-in">
            ...
            <div style={{ background:"rgba(245,197,24,0.06)", border:"1px solid rgba(245,197,24,0.15)", borderRadius:8, padding:"8px 12px", marginBottom:12, fontFamily:"'Inter',sans-serif", fontSize:11, color:"#666" }}>
              ⚡ Schedule refreshes every 10 min · Dimmed matches have no wager stake
            </div>
            {beforeStart ? <Countdown /> : upcomingMatches.length === 0 ? (
              ...
              return (
                <div key={m.id} className="match-card" style={{
                  background: isYours ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.015)",
                  border: "1px solid rgba(255,255,255,0.07)",
                  opacity: isYours ? 1 : 0.45
                }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8, flexWrap:"wrap" }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:"'Inter',sans-serif", fontSize:13, color:"#fff", fontWeight:600 }}>{m.home} vs {m.away}</div>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
                    <div>
                      <div style={{ fontFamily:"'Inter',sans-serif", fontSize:13, color:"#fff", fontWeight:600 }}>
                        {r.home} {r.score ? <span style={{ color:"#888" }}>({r.score})</span> : ""} vs {r.away}
                      </div>
